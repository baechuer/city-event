...
cleanup_smoke() {
  ...
  if command -v powershell.exe >/dev/null 2>&1; then
    powershell.exe -NoProfile -Command "\$target = (Resolve-Path 'tmp/media-service.exe').Path; Get-CimInstance Win32_Process -Filter \"Name = 'media-service.exe'\" | Where-Object { \$_.ExecutablePath -eq \$target } | ForEach-Object { Stop-Process -Id \$_.ProcessId -Force }" >/dev/null 2>&1 || true
  fi
}

port=18087
if command -v cmd.exe >/dev/null 2>&1; then
  smoke_binary="$(windows_binary_path "$REPO_ROOT/tmp/media-service.exe")"
  cmd.exe /d /c "set MEDIA_SERVICE_HTTP_ADDR=127.0.0.1:$port && set POSTGRES_URL=$postgres_url && set MINIO_ENDPOINT=$minio_endpoint && set MINIO_ACCESS_KEY=$minio_access_key && set MINIO_SECRET_KEY=$minio_secret_key && set MINIO_BUCKET=$minio_bucket && $smoke_binary" >tmp/media-service-smoke.log 2>&1 &
else
...
detail="$(http_curl -fsS "http://127.0.0.1:$port/v1/media/$asset_id")"
