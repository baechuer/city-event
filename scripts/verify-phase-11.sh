...
require_contains README.md "documented HA deferral"
...
require_contains deploy/kubernetes/README.md "They do not prove high availability"
require_contains deploy/kubernetes/README.md "high-availability-decision.md"

[[ "$(count_occurrences deploy/kubernetes/deployments.yaml "replicas: 1")" -eq 10 ]] || die "Phase 11 deferral expects ten single-replica readiness Deployments."
require_not_contains deploy/kubernetes/deployments.yaml "kind: HorizontalPodAutoscaler"
require_not_contains deploy/kubernetes/deployments.yaml "kind: PodDisruptionBudget"
