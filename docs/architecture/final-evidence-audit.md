...
- Redis as a non-authoritative feed cache
...
```text
Added Redis-backed feed caching as a non-authoritative fast path with Postgres fallback.
```
...
```text
Prepared the services for Kubernetes deployment with container builds, Deployments, Services, health probes, configuration separation, Secret templates, resource limits, and documented high-availability requirements.
```
...
```text
correct transactional core + at-least-once messaging + idempotent business effects + Redis caching + observability + Kubernetes readiness
```
